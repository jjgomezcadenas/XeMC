...
\begin{description}[leftmargin=2.0cm,style=nextline]
\item[Barrel] Three contributing sources: OCV barrel $\to$ vacuum $\to$ ICV
barrel (compound), MLI insulation $\to$ ICV barrel (transparent source,
propagated through ICV only), and ICV barrel self-emission. Each produces its
own flux table; an activity-weighted sum gives the combined rate.
...
This surface is called the \emph{virtual envelope}. It is defined by the
tracking detector boundary, not by the source geometry, to guarantee that every
sampled gamma starts inside the detector's tracking volume.

The virtual envelope is formalized as a \texttt{VirtualEnvelope} struct with
explicit, inspectable parameters extracted from the \texttt{FastKernelGeometry}
at construction time. A small inset (0.01\,cm) is applied to avoid
floating-point boundary issues with the fast-kernel region classifier.

\begin{center}
\begin{tabular}{llrl}
\toprule
Surface & Kind & $R$ [cm] & Reference region \\
\midrule
Barrel & cylinder & 82.09 & \texttt{LZ\_detector} barrel \\
Top & cap (up) & 82.09 & \texttt{AirDome} \\
Bottom & cap (down) & 82.09 & \texttt{LXe\_passive} bottom cap \\
\bottomrule
\end{tabular}
\end{center}
...
\subsection{Future: internal sources}

The cryostat sources (OCV, MLI, ICV) are the first class of sources
implemented. The following internal sources remain:

\begin{itemize}
\item \textbf{Field cage}: FC\_PTFE (Teflon panels) and FC\_rings (Ti rings).
These emit directly into the LXe with KN source-side transport.
\item \textbf{PMT arrays}: top, bottom, and barrel PMTs plus cables, bases,
and support structures. These are virtual (transparent) sources that emit
at their physical locations without source-side attenuation.
\item \textbf{Grid holders}: FC\_topgrid and FC\_botgrid (stainless steel,
KN transport).
\end{itemize}

Internal sources do not require compound propagation through cryostat layers.
Their flux tables are generated by single-source \texttt{generate\_flux\_bi214}
/ \texttt{generate\_flux\_tl208} and injected directly into the fast kernel via
an appropriate virtual envelope. The existing pipeline handles them with minimal
additional code: one new dispatch entry per source class and one virtual
envelope definition.
...
volumes, uniform position sampling, direction classification.
...
(barrel and ellipsoidal caps), direction reconstruction, gamma-from-flux
sampling.
\item[\texttt{source\_dispatch.jl}] \texttt{VirtualEnvelope},
\texttt{make\_virtual\_envelope}, \texttt{make\_surface\_sampler}, source
dispatch and thread-result merging.
