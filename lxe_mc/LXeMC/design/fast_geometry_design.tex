...
\item The fast kernel is \emph{data-driven}: it reads its region list from
  JSON and identifies the tracking envelope by the \texttt{role} marker,
  not by a hard-coded volume name. The geometry is a strict partition of
  the envelope --- every point inside the envelope belongs to exactly
  one inner region, no fallback. Adding or removing a tracked region
  is a JSON-only change (with the partition contract verified at load
  time by a unit test).
...
\item \texttt{AirDome}, \texttt{AirCyl} — gas regions above the gate (\texttt{tag: vacuum}, filled with \texttt{Vacuum} for straight-line transit)
\item \texttt{LXe\_dome}, \texttt{LXe\_below\_FC}, \texttt{LXe\_below\_cathode} — passive LXe primitives tiling the LXe volume that no active or structural region claims (\texttt{tag: passive\_lxe})
...
through.
\item[\texttt{LXe\_dome}, \texttt{LXe\_below\_FC}, \texttt{LXe\_below\_cathode}]
Three passive-LXe primitives that tile the LXe volume not claimed by
active or structural regions: the bottom ellipsoidal dome
($z \in [-68.7, -41.33]$), the disk below the field-cage envelope
($z \in [-41.33, -13.75]$), and the cylindrical column below the
cathode ($z \in [-13.75, 0]$, $R \le 72.8$\,cm). The last is the
bottom-PMT entry path: photons emitted by the bottom-PMT array must
traverse it before entering BottomActive.
...
\item One distinguished region is identified by marker, not by name:
\begin{itemize}
\item \textbf{Envelope}: the unique region with \texttt{role = "tracking\_envelope"}.
\end{itemize}
The compiler validates that exactly one such region exists. There is no
fallback region: the geometry is required to be a strict partition of
the envelope, verified by the \texttt{FastKernel geometry partition}
testset (10\textsuperscript{5} sampled points, each must classify into
exactly one inner region).
...
order, and returns the first match. If no inner region matches (a partition
gap), it returns \texttt{nothing} --- the transport loop treats this as an
escape.
...
No volume name appears in the classification or compilation logic of
\texttt{compile\_fastkernel\_geometry} or \texttt{classify\_fastkernel}.
However, the transport-level helpers in \texttt{tracking\_fast.jl}
(\texttt{\_visible\_threshold\_MeV}, \texttt{\_is\_passive\_region}) and the
escape classifier in \texttt{tracking.jl}
(\texttt{\_classify\_escape\_volume}) do reference region names for veto
and policy decisions. Adding a new tracked region to the JSON therefore
requires updating these helpers if the region should participate in veto
or escape-volume classification; the geometry compiler and classifier
themselves remain purely data-driven.

\subsection{Full stack inside the FV}

The FV transport uses a \texttt{PhysicalVolume} (\texttt{PCyl}) compiled from
the FV node by \texttt{compile\_fv\_volume}, together with the
\texttt{FastKernelGeometry} for escape-volume classification. The FV stack keeps:
...
\paragraph{Implemented: PMT bottom with passive LXe.}
The \texttt{pmt\_bottom\_lxe} source adds compound propagation through the
passive LXe slab between the bottom PMT array and the cathode. The flux is
recorded at the cathode boundary (the interface between passive and active LXe),
giving the spectrum after attenuation in the passive column.

\paragraph{Implemented: PMT barrel.}
...
wall, merged into a single transparent \texttt{PCylShell} with inward radial
filtering. Same pattern as PMT top/bottom but with a barrel VE.
...
\texttt{pmt\_bottom\_flux}, \texttt{pmt\_bottom\_lxe\_flux},
\texttt{pmt\_barrel\_flux}), merged transparent volume construction with
...
\item \texttt{pmt\_top\_flux},
      ...
      \texttt{pmt\_bottom\_lxe\_flux},
      \texttt{pmt\_barrel\_flux}
